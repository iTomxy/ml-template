...
input: numpy.ndarray or tf.Tensor
output: tf.Tensor
"""


class Resize(L.Layer):
    ...
    def call(self, x):
        return tf.image.resize(x, self.size)
...
class RandomHorizontalFlip(L.Layer):
    def __init__(self):
        super().__init__()
...
class RandomVerticalFlip(L.Layer):
    def __init__(self):
        super().__init__()
...
class Normalize(L.Layer):
    ...
    e.g., a `Lambda(lambda x: x / 255)` might has to be added before this
        if the original image is in range [0, 255].
    """

    def __init__(self, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
        super().__init__()
        self.mean = tf.constant(mean, dtype="float32", shape=[1, 1, 3])
        self.std = tf.constant(std, dtype="float32", shape=[1, 1, 3])
