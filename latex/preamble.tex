% --- colour
\usepackage[table]{xcolor} % \textcolor, \definecolor, \columncolor, \rowcolor
...
\definecolor{carmine}{RGB}{157, 41, 51} % 胭脂 as red
...
% This is for adding footnote after algorithm (FROM: https://arxiv.org/abs/2104.02057)
\usepackage{etoolbox}
\makeatletter
...
% \shline: Strong \hline (FROM: https://arxiv.org/abs/2104.02057)
\newlength\savewidth\newcommand\shline{\noalign{\global\savewidth\arrayrulewidth
  \global\arrayrulewidth 1pt}\hline\noalign{\global\arrayrulewidth\savewidth}}
\usepackage{tabulary}
% Column style `x' (FROM: https://arxiv.org/abs/2104.02057):
% 1) cells have fixed width in pt.
% 2) text inside is centered.
% 3) long text wraps with that fixed width.
\newcolumntype{x}[1]{>{\centering\arraybackslash}p{#1pt}} % column style `x{#1}' for tabular
% \tablestyle (FROM: https://arxiv.org/abs/2104.02057): use before `\begin{tabular}'
\newcommand{\tablestyle}[2]{\setlength{\tabcolsep}{#1}\renewcommand{\arraystretch}{#2}\centering\footnotesize}
% Inline table (FROM: https://arxiv.org/abs/2104.02057), tune the parameters as needed.
...
\usepackage{amssymb} % loads `amsfonts', \mathbb, \checkmark, \lozenge
% \usepackage{amsfonts} % Already loaded by `amssymb'.
% \usepackage{bm} % defines \bm
\usepackage{newtxmath} % hollow char
...
\usepackage{textcomp}
\providecommand{\x}{\texttimes}
\newcommand{\app}{\raise.17ex\hbox{$\scriptstyle\sim$}} % `~' (FROM: https://arxiv.org/abs/2104.02057)
\providecommand{\worse}{\scriptsize{\textsubscript{(\textcolor[RGB]{192, 41, 43}{$\downarrow$})}}} % (↑)
\providecommand{\better}{\scriptsize{\textsubscript{(\textcolor[RGB]{39, 137, 72}{$\uparrow$})}}} % (↓)
...
\usepackage[capitalize]{cleveref} % must AFTER hyperref
